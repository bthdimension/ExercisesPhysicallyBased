...
void SceneEditor::createDebugScene() {
	...
	for (int i = 1; i < 2; i++) {
		...
		vmml::Vector3f pos = vmml::Vector3f(0.0f + (float) (i % 2), j + 1.0f, 0.0f);
		vmml::Vector3f rotAx = vmml::Vector3f(0.f, 0.0f, 0.0f);

		ARigidBodyOctree* rb;
		if (i == 1) {
			rb = new BlockRigidBody(modelptr, pos, rotAx);
			//rb->setAxesRotation(vmml::Vector3f(0.2, 0.3, 0.1));
			rb->setAxesRotation(vmml::Vector3f(0.7854, 0.0, 0.7854));
		} else {
			rb = new SphereRigidBody(modelptr, pos, rotAx);
		}
